 ...
 * Uses mock weather/news APIs; replace with real integrations in production.
 */
...
/**
 * Mock Google Weather API-style weather alert response.
 * @param {string} city - City name
 * @returns {Promise<object[]>}
 */
export async function fetchWeatherAlert(city) {
  await new Promise((r) => setTimeout(r, 150));
  const conditions = ['Clear', 'Rain', 'Heatwave', 'Cyclone Watch'];
  const condition = conditions[Math.floor(Math.random() * conditions.length)];
  const severity = condition === 'Cyclone Watch' ? 'critical' : condition === 'Rain' ? 'medium' : 'low';
  ...
  return [
    {
      ...
      message: `Current conditions in ${city}: ${condition}. Plan accordingly.`,
